...
const generateArray = async (length: number, variant: number) => { 
  ...
  let array: number[] = Array.from({ length }, () => Math.floor(Math.random() * 100));
  switch (variant) {
    case 0:
    ...
    case 2:
      ...
      return array;
    case 3:
      array = Array.from({ length }, () => Math.floor(Math.random() * 1000) / 10);
    ...
    case 4:
      ...
      return array;
    case 8:
      const names = await Promise.all(Array(length).fill(0).map(getName)) as string[];
    ...
    case 10:
      ...
      const result = array.filter(num => num % 2 !== 0).concat(secondArray.filter(num => num % 2 === 0));
      return result;
    case 11:
      // matrix :(
      return array;
    ...
    case 14:
      const cities  = await Promise.all(Array(length).fill(0).map(getCity)) as string[];
    ...
    case 16:
      ...
      const arrayWithMin = array.map((item) => (item < 0 ? item * minEl : item));
      return arrayWithMin;
    case 17:
