...
export const quickSort: Function = async (
  ...
  for(let i = 0; i < arr.length; i++) {
    console.warn(arr[i], getSum(arr[i] as number[]));
  }
...
async function swapLocal(
  ...
  for(let i = 0; i < items.length; i++) {
    console.warn(items[i], getSum(items[i] as number[]));
  }
}
...
async function partition(
  ...
  if (Array.isArray(items[0])) {
    ...
    let pivot = getSum(matrix[Math.floor((right + left) / 2)]); //middle elemen
    console.log(pivot);
    let i = left; //left pointer
    let j = right; //right pointer
    while (i <= j) {
      while (getSum(matrix[i]) < pivot) {
        i++;
      }
      while (getSum(matrix[j]) > pivot) {
        j--;
      }
      if (i <= j) {
